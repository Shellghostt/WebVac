...
from auth.auth import AuthHandler, CAPTCHA_SELECTORS
from auth.cookie_audit import audit_cookies, print_audit_warnings
from auth.credentials import resolve_credentials
from auth.mfa import generate_totp, prompt_manual_challenge, prompt_otp
from auth.nodriver_auth import NodriverAuthHandler
from auth.profile import AuthProfile, load_auth_profile, merge_cli_into_profile
...
from auth.steps import run_steps_nodriver, run_steps_patchright
from auth.wall import apply_wall_policy, is_auth_wall, is_auth_wall_page, is_logout_url
...
class AuthManager:
    ...
    async def login(self, *, seed_url: str = "") -> bool:
        ...
        engine = (self.profile.auth_engine or "patchright").lower()
        if engine == "nodriver":
            ok = await self._login_nodriver(login_url)
        else:
    ...
    async def _login_nodriver(self, login_url: str) -> bool:
        session_file = self.profile.session_file
        if not session_file:
            host = urlparse(login_url).netloc.replace(":", "_") or "session"
            session_file = os.path.join("sessions", f"{host}_nodriver.json")
            self.profile.session_file = session_file

        if self.profile.steps:
            # Nodriver custom steps path
            try:
                import nodriver as uc
            except Exception as exc:
                print(f"[Auth/Nodriver] import failed: {exc}")
                return False
            browser = await uc.start(headless=self.headless)
            try:
                tab = await browser.get(login_url)
                await tab.sleep(1.0)
                ok = await run_steps_nodriver(
                    tab,
                    self.profile.steps,
                    username=self.profile.username,
                    password=self.profile.password,
                ...
                if not ok:
                    return False
                await tab.sleep(1.5)
                # Export cookies via NodriverAuthHandler helper
                nd = NodriverAuthHandler()
                cookies = await nd._get_cookies_dicts(browser, tab)
                if not cookies:
                    print("[Auth/Nodriver] No cookies after steps login.")
                    return False
                os.makedirs(os.path.dirname(session_file) or ".", exist_ok=True)
                # Save as storage_state-shaped via session_store
                save_session(
                    session_file,
        ...
        return await nd.login(
            ...
            timeout=self.timeout,
            username_selector=self.profile.username_selector,
            password_selector=self.profile.password_selector,
            submit_selector=self.profile.submit_selector,
            dismiss_selectors=self.profile.dismiss_selectors or None,
        )
...
def build_profile_from_args(args, *, profile_path: Optional[str] = None) -> AuthProfile:
    ...
    return merge_cli_into_profile(
        ...
        auth_engine=getattr(args, "auth_engine", None) or profile.auth_engine,
