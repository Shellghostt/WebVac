...
**Biggest gap:** WebVac does not yet expose a dedicated MCP-friendly job API for remote BAS orchestration.
...
2. **Endpoint discovery** — crawl links and capture useful page/session metadata  
3. **Session/cookie intelligence** — cookies + storage artifacts (values redacted in MCP responses)  
4. **Structured JSON contracts** — pages, endpoints, session metadata, job status (not HTML reports)  
...
10. **Fail-closed auth** — failed login or auth-wall → job error (not “continue anyway” / silent skip)
...
Demote desktop UX/export features in the remote server path and keep the core scraper focused on stable machine-readable output.
...
### 4.3 Library API (MCP wraps this — not CLI subprocess)

```text
start_job(config) -> job_id
get_status(job_id) -> phase, pages_done, errors
cancel_job(job_id)
get_result(job_id) -> {pages, endpoints, session_meta, artifacts?}
```
...
| `webvac_start_scan` | Crawl job (caps, auth session) |
| `webvac_scan_status` | Progress / phase / errors |
| `webvac_cancel_scan` | Cooperative cancel |
| `webvac_get_endpoints` | Discovered URLs and crawl graph data |
...
auth_restore | auth_login
  → start_scan(on_auth_wall=abort)
  → poll status
  → get_endpoints + get_session_meta
```
...
2. **Origin IP (`--origin-ip`)** — can bypass CDN/WAF; manual only; require explicit permission.  
3. **Auth-wall skip** — under-reports authenticated surface; wrong for simulations.  
4. **Resource abuse** — high concurrency / unlimited pages can look like DoS.  
5. **Secrets** — never log passwords; encrypt sessions; inject via env/secret store.
...
| P2 | Gate manual origin IP behind policy flags |
...
**Keep** the crawl + AuthManager core.  
**Wire** scrape results into a job API.  
