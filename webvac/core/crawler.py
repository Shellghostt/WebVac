...
class Crawler:
    ...
    async def _handle_consent(self, page: Page, url: str = "") -> None:
        """Auto-dismiss CMP banners; optional headed pause once per host."""
        from webvac.utils.consent import handle_page_consent

        host = ""
        try:
            host = urlparse(url or getattr(page, "url", "") or "").netloc.lower()
        except Exception:
            host = ""

        already = bool(host and host in self._consent_paused_hosts)
        pause = bool(self.session_config.get("pause_for_consent"))
        result = await handle_page_consent(
            page,
            url=url,
            extra_selectors=self.session_config.get("dismiss_selectors") or None,
            dismiss=bool(self.session_config.get("consent_dismiss", True)),
            pause_for_consent=pause,
            headless=bool(getattr(self.browser, "headless", True)),
            already_paused=already,
        )
        if result.get("clicked"):
            tqdm.write(f"[Consent] Dismissed {result['clicked']} control(s) on {host or url}")
        if result.get("paused") and host:
            self._consent_paused_hosts.add(host)
